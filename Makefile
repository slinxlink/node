...
slinx-amd64:
	@$(DOCKER) "mkdir -p /app/dist && \
		...
		cd /app && GOOS=linux GOARCH=amd64 CGO_ENABLED=1 CC=x86_64-linux-gnu-gcc go build -ldflags='-s -w' -o dist/slinx_linux_amd64 . && \
...
slinx-arm64:
	@$(DOCKER) "mkdir -p /app/dist && \
		...
		cd /app && GOOS=linux GOARCH=arm64 CGO_ENABLED=1 CC=aarch64-linux-gnu-gcc go build -ldflags='-s -w' -o dist/slinx_linux_arm64 . && \
